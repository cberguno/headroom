...
if __name__ == "__main__":
   ...
   # Make proofs_dname required for imo_proof_grading
    ...
    # Alpaca paper-trading domain (train/val/test are real chronological
    # windows over cached historical data; see domains/alpaca_trading/).
        ...
        output_folder = harness_alpaca_trading(
            agent_path=args.agent_path,
            output_dir=args.output_dir,
            run_id=args.run_id if args.run_id is not None else datetime.now().strftime("%Y%m%d_%H%M%S_%f"),
            num_samples=args.num_samples,
            subset=args.subset,
            resume_from=args.resume_from,
        )
